...
"""# 📌 🌎 Region Extraction & Normalization"""
...
def normalize_region(region_text):
    """
    Normalize broad regions → list of countries or Unknown,
...
"""#  ⚙️ Signal Scoring & Processing Loop
...
# 🐛 Optional: Print the last 3 scored signals for debug
for sig in scored_signals[-3:]:
    print(sig)

"""# 🔗 Google Sheets Connection & Deduplication
...
spreadsheet_id = '1WyDkJ1CN8sdTDWDhBzMIYSeCEPkruq5rYZIJfcHYjt0'
worksheet = gc.open_by_key(spreadsheet_id).sheet1
set_with_dataframe(worksheet, df_classified)

# === Load existing signals from the sheet for deduplication ===
import pandas as pd
...
try:
    archive_sheet = gc.open_by_key(spreadsheet_id).worksheet("AllSignalsArchive")

    for signal in classified_signals:
        row = [
            signal.get("timestamp", ""),
            signal.get("raw_value", ""),
            signal.get("classified_event", ""),
            signal.get("region", ""),
            signal.get("confidence_flag", ""),
            signal.get("final_score", ""),
            signal.get("source", ""),
            signal.get("matcher_name", ""),
            signal.get("category", ""),
            signal.get("region_confidence", ""),
            signal.get("region_precision", ""),
            signal.get("heartbeat", ""),
            signal.get("credibility_score", ""),
            signal.get("urgency_score", ""),
            signal.get("historical_sensitivity_score", ""),
            signal.get("base_final_score", ""),
            signal.get("penalty_reason", ""),
            signal.get("score_penalty", ""),
            signal.get("schema_version", ""),
            signal.get("last_updated", ""),
        ]
        archive_sheet.append_row(row)
    print("📦 Full signal archive updated.")
except Exception as e:
...
worksheet = gc.open_by_key(spreadsheet_id).sheet1
set_with_dataframe(worksheet, df_classified)
print("[INFO] Signals classified and exported to Google Sheets.")

# === FORWARD TO INDUSTRY ENGINE SHEET IF FLAGGED ===
try:
    industry_engine_sheet = gc.open_by_key(spreadsheet_id).worksheet("IndustryEngineInput")
    for signal in classified_signals:
        if signal.get("forward_to_engine"):
            row = [
                signal.get("timestamp", ""),
                signal.get("raw_value", ""),
                signal.get("classified_event", ""),
            ...
            industry_engine_sheet.append_row(row)
            print(f"✅ Forwarded: {signal.get('classified_event')} - {signal.get('region')}")
except Exception as e:
